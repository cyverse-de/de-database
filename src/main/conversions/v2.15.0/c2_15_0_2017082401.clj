...
(defn- update-url-import-tool
  ...
  (sql/update :tools
              ...
              (sql/where {:id (UUID/fromString "681251EF-EE59-4FE9-9436-DC8A23FEB11A")}))
  (sql/update :apps
              (sql/set-fields {:name "Url Import"
                               :description "A Go tool for DE URL imports"})
              (sql/where {:id (UUID/fromString "1E8F719B-0452-4D39-A2F3-8714793EE3E6")}))
  (sql/update :parameters
              (sql/set-fields {:name "-filename"})
              (sql/where {:id (UUID/fromString "1DD009B1-CE1E-4933-ABA8-66314757288B")}))

  (sql/update :parameters
              (sql/set-fields {:name "-url"})
              (sql/where {:id (UUID/fromString "A0D6A102-8623-47B9-A57F-224D6A71F28D")}))
  ...
  (sql/update :container_images
              ...
              (sql/where {:id (UUID/fromString "15959300-b972-4571-ace2-081af0909599")})))
